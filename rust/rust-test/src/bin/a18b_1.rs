...
//   2. Find the employee with the 'find_ employee' database function
...
enum PortectedLocation {
    All,
    Office,
    Warehouse,
}

impl PortectedLocation {
...
impl Database {
    ...
    fn find_employee(&self, name: &str) -> Result<Employee, String> {
        match name {
            ...
            "Brody" => Ok(Employee { name: "Brody".to_owned() }),
    ...
    fn get_keycard(&self, employee: &Employee) -> Result<KeyCard, String> {
        match employee.name.as_str() {
            "Anita" => Ok(KeyCard { access_level: 1009 }),
...
fn authorize(employee_name: &str, location: PortectedLocation) -> Result<AuthrizationStatus, String> {
    // will fail if connect returns Err
    ...
    */
    ...
    if keycard.access_level >= location.required_access_level() {
        return Ok(AuthrizationStatus::Allow);
    } else {
        return Ok(AuthrizationStatus::Deny);
    }
}

fn main() {
    let results = vec![
        authorize("Anita", PortectedLocation::Warehouse),
        authorize("Brody", PortectedLocation::Office),
        authorize("Catherine", PortectedLocation::All),
    ];
