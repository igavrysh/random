...
fn total_cost(m: &Vec<Box<dyn Material>>) -> f64 {
    m.iter().map(|mat| mat.total_cost()).sum()
}
...
fn main() {
    let m: Vec<Box<dyn Material>> = vec![
        Box::new(Wood(32f64)),
        Box::new(Carpet(32f64)),
        Box::new(Tile(123f64)),
    ];
    let total_cost = total_cost(&m);
